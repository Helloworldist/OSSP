...
page = int(input("페이지 범위 입력 : "))

# 2단계
f_count = int(input("2. 데이터를 저장할 항목 개수를 입력하세요(최대 7개) : "))


# 3단계
main_structure = input("3.1 메인 Xpath 설정 :")
print("3.2 서브 Xpath 설정")
for i in range(0, f_count, 1):
    xpath = input("%d번째 필드에 대한 Xpath 입력 : " % int(i+1))
    sub_arr[i] = xpath
